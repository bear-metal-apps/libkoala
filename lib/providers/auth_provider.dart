...
class Auth {
  ...
  Future<void> login(List<String> scopes) async {
    ...
    try {
      ...
      final requestScopes = {...scopes, 'offline_access', 'openid', 'profile'}
          .join(' ');
  ...
  Future<OAuthToken> _fetchNewToken(String refreshToken, List<String> scopes) async {
    return _postRequest({
      'client_id': _clientId,
  ...
  Future<OAuthToken> _postRequest(Map<String, String> body) async {
    ...
    if (response.statusCode != 200) {
      ...
      throw Exception('Auth Error: ${error['error_description'] ?? response.body}');
    }
